...
adapt_deriv <- function(Theta,lambda,gamma = 1){
  Theta <- (abs(Theta) + 1e-2)
  lambda_mat <- lambda * Theta^(-gamma)
  return(lambda_mat)
}

# Fan, J., & Li, R. (2001). Variable selection via non concave penalized likelihood and
...
# Zhang, C. H. (2010). Nearly unbiased variable selection under mini-max concave penalty.
