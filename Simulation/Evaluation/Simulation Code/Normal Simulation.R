...
Separate.simul <- function(n, dimen, gam = 1,Heavy = "Matrix",
                           penalty = 'adapt',Omega,
                           scale = NULL,seed = 14, norm.off.diag = FALSE,
                           rate = c(1e-5,1e-6),
  ...
  Separate <- foreach(j = 1:Run, .combine =rbind,
                      .packages = c("tensr","ramify","stats",'rTensor','expm',
                                    "Tlasso","glasso","MASS"))%dopar%{
                                      source("~/Basic Setting.R")
                                      source("~/Simulation Evaluation.R")
                                      source("~/Simulation/Evaluation/Separate.R")
                                      source("~/Simulation/Data Generation.R")
                                      Data <- Generate.Normal(n = n, dimen = dimen, seed = seed * j,
                                                              Omega = Omega)
...
Cyclic.simul <- function(n,dimen,Omega, seed = 14,
                         gam = 1, rate = c(1e-5,1e-6),
                         penalty = 'adapt',Run = 100, Heavy = "Matrix",
                         thres = 1e-5,
                         t = 5,sparse = c(1,2),
                         norm.off.diag = FALSE){
  Cyclic <- foreach(j = 1:Run, .combine =rbind,
                    .packages = c("tensr","ramify","stats",'rTensor','expm',
                                  "Tlasso","glasso","MASS"))%dopar%{
                                    source("~/Basic Setting.R")
                                    source("~/Simulation Evaluation.R")
                                    source("~/Simulation/Evaluation/Tlasso.R")
                                    source("~/Simulation/Data Generation.R")
                                    Data <- Generate.Normal(n = n, dimen = dimen, seed = seed * j,
                                                            Omega = Omega)
                                    X <- Data$Cyclic.X;Vax <- Data$Cyclic.Vax
                                    ...
                                    if(penalty == 'adapt'){
                                      fit <- Tlasso.edit(x = x,val = val,lambda.min.ratio = rate[1],thres = thres)
                                      fit.adapt <- Tlasso.adapt(x = x,val = val, thres = thres,
                                                                Omega = fit$Omegahat,rate = rate[2])
                                      out <- Matrix.simulation.summary(Omega.hat.list = fit.adapt$Omegahat,
                                                                       Omega.true.list = Omega,
                                                                       norm.off.diag = norm.off.diag)
                                    }else{
                                      fit <- Tlasso.edit(x = x,val = val,lambda.min.ratio = rate[1])
                                      out <- Matrix.simulation.summary(Omega.hat.list = fit$Omegahat,
                                                                       Omega.true.list = Omega,
                                                                       norm.off.diag = norm.off.diag)
                                    }
...
Gemini.simul <- function(n, dimen, Omega,scale = NULL,gam = 1,
                         penalty = 'adapt', rate = c(1e-5,1e-6),
  ...
  Gemini <- foreach(j = 1:Run, .combine =rbind,
                    .packages = c("tensr","ramify","stats",'rTensor','expm',
                                  "Tlasso","glasso","MASS"))%dopar%{
                                    source("~/Basic Setting.R")
                                    source("~/Simulation Evaluation.R")
                                    source("~/Simulation/Evaluation/Gemini.R")
                                    source("~/Simulation/Data Generation.R")
                                    Data <- Generate.Normal(n = n, dimen = dimen, seed = seed * j,
                                                            Omega = Omega)
                                    X <- Data$Cyclic.X;Vax <- Data$Cyclic.Vax
